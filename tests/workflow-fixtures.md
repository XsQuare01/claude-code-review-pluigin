...
| 38 | C-7 | Any finding emitted as a `####` heading under the C-7 skeleton (today: `/code-review-full`) | The line under the heading reads `영향: {높음\|낮음} · 확신: {높음\|낮음}`, and the severity marker matches the derivation in `00-rule.md` — 🔴 only when both are 높음 |
| 39 | C-7 | A finding that is true but has no user-visible effect — style, consistency, a theoretical inefficiency | It carries `영향: 낮음`, so it is 🟡 when `확신: 높음` and 🔵 when `확신: 낮음` — never 🔴; it is still reported rather than dropped |
