...
export function checkSectionContent(reportText, findings) {
  ...
  //
  // 이 검사는 세 번 틀렸고, 세 실패가 경계를 정한다.
  //
  // **너무 좁았을 때** — "라벨 뒤 12자 안의 숫자"가 코드 인용 속 문자 클래스
  // `[0-9]`에 먼저 걸려, 개수를 제대로 적은 리포트를 실패시켰다.
  //
  //   - **모듈 후보**: `ls "$RULES_DIR"/[0-9]*.md` 결과 22개 → non-00 21개
  //
  // **너무 넓었을 때 (1)** — "그 줄에 숫자가 있나"로 완화하자 그 `[0-9]`의
  // 존재 자체가 개수의 증거가 됐다.
  //
  // **너무 넓었을 때 (2)** — "그 줄 어딘가에 개수 표현이 있나"로 고쳤더니
  // **다른 것의 개수**여도 통과했다. 라벨과 숫자가 같은 줄에 있다는 것과 그
  // 숫자가 그 라벨의 개수라는 것은 다른 명제다.
  //
  //   - 후보 수는 기록하지 않았고 실패 2건      ← 2는 실패 수다
  //   - 후보는 2단계에서 추린다                  ← 2는 순서다
  //   - 후보 산정은 2026-08-28 기준이다          ← 2026은 연도다
  //
  // 세 실패의 공통점은 **결합**을 안 봤다는 것이다. 그래서 라벨과 개수가
  // 어떻게 묶였는지로 판정하고, 창의 폭을 결합 방식에 따라 다르게 준다.
  //
  //   `후보: …`  — 콜론이 붙으면 **필드 선언**이다. 값이 멀리 있어도 그
  //                 라벨의 값이므로 창을 넓게 준다
  //   `후보 21개` — 콜론이 없으면 산문이다. 붙어 있을 때만 그 라벨의 개수로
  //                 인정한다. 창은 `**후보 모듈** 21개`처럼 라벨과 수 사이에
  //                 한정어가 끼는 실물 형태까지만 넓힌다
  //
  // 개수 표현은 셋이다. 맨 수는 앞뒤를 함께 막아야 개수와 개수 아닌 것이
  // 갈린다 — 뒤에 한글이면 단위(`2단계`), 뒤에 `-`나 `.`면 날짜·버전
  // (`2026-08-28`), 앞에 영문자면 버전(`v2`)이다. 어느 것도 개수가 아니다.
  // 문자 클래스는 미리 지운다. `[0-9]`가 첫 실패의 원인이었고, 코드 인용을
  // 통째로 지우면 `\`N=20\`` 같은 정당한 개수까지 사라진다.
  //
  // SKIPPED 목록의 완결성은 여전히 검사하지 않는다. 후보와 적용 수를 어느
  // 숫자로 읽어야 하는지가 형태마다 달라(첫 형태는 22와 21이 함께 나온다)
  // 신뢰할 수 없고, **믿을 수 없는 검사는 검사가 없는 것보다 나쁘다.**
  {
    const lines = sectionBetween(reportText, '## 실행 계획')
    const COUNT = String.raw`(?:\d+\s*(?:개|건)|[A-Za-z]\s*=\s*\d+|(?<![A-Za-z])\d+(?![가-힣\d\-.]))`
    const FIELD_WINDOW = 40
    const PROSE_WINDOW = 8
    const reports = label => lines.some(line => {
      const text = line.replace(/\[[^\]]*\]/g, ' ')
      if (!text.includes(label)) return false
      const declaresField = new RegExp(`${label}[^\\n]{0,3}?[:=]`).test(text)
      const window = declaresField ? FIELD_WINDOW : PROSE_WINDOW
      const after = new RegExp(`${label}[^\\n]{0,${window}}?${COUNT}`)
      // 라벨 앞에 오는 형태도 있다: `21개 중 19개 적용`.
      const before = new RegExp(`${COUNT}[^\\n]{0,${PROSE_WINDOW}}?${label}`)
