...
표를 낸 **뒤에** 위치 목록을 아래 블록으로 함께 내세요. 표가 주 출력이고 이 블록은 부속입니다 — 표를 줄이거나 바꾸지 마세요.

```
<!-- REVIEW_LOCATIONS:BEGIN -->
[
  {"ruleId": "03-1", "path": "src/hooks/use-camera.ts", "line": 42, "quote": "if (pending) return"}
]
<!-- REVIEW_LOCATIONS:END -->
```

- 표에 적은 각 지적마다 한 행씩, 네 필드만 담습니다. 본문·개선 제안·등급은 넣지 마세요 — 이미 표에 있습니다
- `quote`는 그 줄의 코드를 **그대로** 적습니다. 표의 인용과 같은 값입니다
- 위치를 확인하지 못한 지적은 **행을 넣지 마세요.** 추측한 줄 번호를 넣는 것보다 빠뜨리는 편이 낫습니다
- 이 블록을 내지 못하겠으면 **생략하세요.** 생략은 실패가 아닙니다
...
### Step 5: 결과 수집 및 위치 대조

bounded pass 완료 후:

1. sub-agent 결과를 확인한다
2. `REVIEW_LOCATIONS` 블록이 있으면 아래 위치 대조를 수행한다
3. 결과를 derived severity 순서로 병합: 🔴 → 🟡 → 🔵
4. 같은 severity 내에서는 모듈 순서대로 정렬
5. 위반 없는 모듈은 최하단에 "✅ 통과" 로 요약
...
- 스크립트는 저장소 밖 경로, `..`, `.git/**`을 읽지 않는다. 그런 경로는 `location-unresolvable`로 돌아온다

**대조를 못 한 경우** — 블록이 없거나, JSON이 깨졌거나, 스크립트를 찾지 못했거나, 돌려받은 행 수가 보낸 행 수와 다르면 **`대조 미실행`으로 적고 넘어간다.**

- 교정 재시도를 하지 않는다. 이 블록은 부속이고, 재시도는 통합 패스를 타임아웃으로 밀어넣은 원인 중 하나였다
- 지적은 전부 보존한다
- **완전한 위치 검증을 수행한 것처럼 서술하지 않는다**
