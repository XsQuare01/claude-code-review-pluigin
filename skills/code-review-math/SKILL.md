...
- 같은 폴더의 숫자 prefix 모듈(`00~11`) 및 `fast.md`는 **참조하지 않는다**
...
- `C:\\Users\\bhmun\\.claude\\review-rules\\math.md`

이 문서 하나만 사용합니다. `~/.claude/review-rules/` 의 숫자 prefix 모듈(00~11)과 `fast.md`는 참조하지 마세요.
...
리포트는 기본적으로 `C:\Users\bhmun\OneDrive\바탕 화면\Docs\code-review-math-{branch-name}-{date}.md`로 저장하고 경로를 보고한다. 문서 내용은 **이번 브랜치 diff 안에서 행렬 연산이 바뀐 파일과 그 수학적 이슈** 중심으로 쓴다. 기존 리뷰 문서가 이미 있어도 그 문서를 이유로 리뷰를 건너뛰지 말고 **항상 새 리뷰를 수행한 뒤 새 파일로 저장**한다. `workflow-name`은 `math`다.
