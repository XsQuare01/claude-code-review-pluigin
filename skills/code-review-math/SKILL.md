...
- shape/차원 추적, storage order, transpose/inverse 전제, projection 조건 같은 도메인별 근거는 새 필드를 만들지 말고 `body`, `recommendation`, `evidence`, `reason`에 담으세요.
- 이 structured-v1 producer instruction의 owner는 이 skill입니다. `math.md`는 workflow-neutral domain guidance만 제공하고, shared manifest와 lifecycle은 `workflow-contract.md`를 따릅니다.
 )
```
...
- H1은 `# {대상} 수학 코드 리뷰 리포트` 형식으로 target-bearing title을 사용한다.
