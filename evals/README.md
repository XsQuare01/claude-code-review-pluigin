...
## 돌리지 않는다 — 사용자 지시 (2026-08-31)

**`claude`를 실제로 부르는 모드로 이 harness를 실행하지 않는다.** 백그라운드든
포그라운드든, 워크트리든 본 저장소든, 사용자가 그 실행을 명시적으로 요청하지 않는
한 돌리지 않는다.

> 절대 다시는 직접 리뷰 테스트로 돌리지 마. 토큰이 엄청나게 소모돼서 안 돼.

`render-throughput` run 한 번이 40분과 그만큼의 사용량을 쓰고 채점 가능한 결과를
**0건** 남겼다 — 타임아웃으로 죽어 JSON 봉투가 없었다. `/code-review-full`은
fan-out에 특수 패스와 교차검증까지 붙어 run 비용이 크고, 타임아웃이면 그 비용이
전부 버려진다.
...
| 하려는 일 | 쓰는 것 | 비용 |
|---|---|---|
| fixture가 제대로 만들어지는지 본다 | `--dry-run` | 없음 |
| 채점 축을 고치고 확인한다 | `--regrade` (저장된 리포트 재채점) | 없음 |
| 로직을 확인한다 | `node --test tests/*.test.mjs` | 없음 |
| 새 run이 꼭 필요하다 | **명령어를 사용자에게 넘긴다** | 사용자가 판단 |

전문: `C:/Users/bhmun/OneDrive/바탕 화면/Docs/2026-08-31-render-timeout-reproduction.md`
...
완화도 두 번 더 틀렸다. "그 줄에 숫자가 있나"로 바꾸자 그 `[0-9]`의 존재
자체가 개수의 증거가 됐고, "그 줄 어딘가에 개수 표현이 있나"로 고치자
**다른 것의 개수**여도 통과했다 — `후보 수는 기록하지 않았고 실패 2건`의
2는 실패 수다.

지금은 라벨과 개수가 **어떻게 묶였는지**를 본다. 문자 클래스를 먼저 지우고
(코드 인용을 통째로 지우면 `` 같은 정당한 개수까지 사라진다),
라벨 뒤에 콜론이 붙으면 **필드 선언**으로 보아 값이 멀리 있어도 인정하고,
콜론이 없으면 산문으로 보아 붙어 있을 때만 인정한다. 맨 수는 뒤에 한글이
붙지 않을 때만 개수다 — `후보 20`은 개수이고 `후보는 2단계`는 순서다.

함께 포기한 것: 건너뛴 모듈에 사유가 다 붙었는지 검사하는 것. 어느 숫자가
후보 수인지가 형태마다 달라(위 줄은 22와 21이 함께 나온다) 신뢰할 수 없었고,
