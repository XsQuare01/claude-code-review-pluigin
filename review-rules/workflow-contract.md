...
문서의 H1은 **어느 워크플로우가 만든 리포트인지 드러내야** 하고, 저장된 파일명의 `workflow-name`과 일치해야 한다. 제목과 파일명이 어긋나면 리포트를 나중에 열었을 때 어떤 범위로 검토된 것인지 알 수 없다.
...
| `fast` | `# 빠른 코드 리뷰 리포트` |
| `commit` | `# 커밋 코드 리뷰 리포트` |
| `props` | `# Props/인자 전달 코드 리뷰 리포트` |
| `math` | `# 수학 코드 리뷰 리포트` |
| `exception` | `# 예외 처리 코드 리뷰 리포트` |
