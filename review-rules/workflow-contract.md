...
**하위 에이전트가 만든 헤딩을 그대로 옮기지 않는다.** 모듈 sub-agent를 쓰는 워크플로우에서, 에이전트 출력에 `#`~`###` 헤딩이 있으면 오케스트레이터가 이 골격에 맞게 낮춘다. 정규화 대상은 형식뿐이며, 지적의 내용·개수·severity·출처 라벨은 그대로 보존한다.
...
- **두 축은 지적을 만든 주체가 판정한다.** 코드를 읽은 것이 그쪽이기 때문이다. severity는 계산이므로 오케스트레이터가 정규화 단계에서 함께 처리하며, **이를 위해 별도 검증 패스를 추가하지 않는다**

근거는 각 축에서 문제가 되는 쪽에만 요구한다.

| 값 | 근거 | 형식 |
|---|---|---|
| 영향 높음 | **필요** — 네 항목 중 무엇인지 지목 | `영향: 높음 (데이터 손상)` |
| 영향 낮음 | 불필요 | `영향: 낮음` |
...
**이 표기는 등록된 7개 워크플로우 전부에 적용된다.** 파일당 최대 1개만 출력하는 `fast`도 그 1개에 두 축을 붙인다. 계약을 한 곳에 두는 이유가 워크플로우마다 다르게 동작하는 것을 막기 위해서이고, 축 표기도 같은 이유로 공통이다.
