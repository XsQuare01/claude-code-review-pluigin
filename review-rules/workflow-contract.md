...
`--check`는 가장 긴 무기록 구간도 함께 낸다. **그 사이 돌고 있던 모듈 수를 붙여서**
낸다 — 모듈 넷이 나란히 도는 동안 줄이 안 남는 것은 정상이고, 그것을 아무것도 돌지
않은 구간과 같은 문장으로 부르면 뒤쪽이 묻힌다.

**`--summary`는 디스패치의 모양도 낸다** — 모듈 수, 소요 합계, 벽시계, 실효 동시
실행 수, 슬롯이 통째로 빈 시간. 합계와 벽시계가 따로 있어야 "느렸다"와 "놀았다"가
갈린다. 인플라이트가 0으로 떨어진 횟수도 함께 나오는데, **한 모듈이 끝나면 즉시 다음을
넣는다**는 규칙(배리어 금지)을 지킨 실행이라면 그 값은 0이어야 한다. 2026-09-17
실행은 6번 떨어졌고 그동안 333초가 쌓였다 — 지시는 적혀 있었고 지켜지지 않았으며,
세어 보기 전까지 아무도 몰랐다.
