...
**아래 블록이 finding 표기의 정본이다.** producer schema를 `REVIEW_RESULT_CONTRACT_V1`로 주입하듯, renderer를 가진 워크플로우는 이 블록을 **전문 그대로** 렌더 단계 지시에 주입한다 — 요약본이나 산문 설명으로 대체하지 않는다.

이유는 실측이다. 같은 명령이 같은 날 세 번 돌아 **세 가지 다른 표기**를 냈다: `####` 블록, 그리고 `| Severity | Rule | Summary |` 표에 `Error`/`Warning` 영어 등급. 형식을 산문으로만 적어두면 모델이 매번 모양을 재구성하고 매번 다른 답을 낸다. 보여주지 않은 형식은 지켜지지 않는다.

<!-- FINDING_RENDER_SHAPE:BEGIN -->
```
...
```
<!-- FINDING_RENDER_SHAPE:END -->

**고정되는 것 — 이 다섯은 선택지가 아니다.**

- finding 하나는 `####` 헤딩 하나다. **표 행으로 쓰지 않는다.** 표로 압축하면 아래 세 줄이 갈 자리가 없어진다
- severity는 `🔴` `🟡` `🔵` 셋뿐이다. `Error`/`Warning`/`Info`나 `오류`/`경고` 같은 다른 어휘를 쓰지 않는다
- 헤딩 바로 다음 줄은 **축 줄**이다: `영향: … · 확신: …`. 교차검증을 하는 워크플로우는 `· 교차검증: …`를 덧붙인다. severity가 이 두 축에서 파생되므로(`00-rule.md` **Severity 기준**), 축이 없으면 등급의 근거가 리포트에 없다
- 그다음 줄은 **위치와 인용**이다: `` `경로:줄` — `그 줄의 코드` ``. `00-rule.md` 00-10을 따른다. 기능 이름이나 서술("binary WebSocket message handler")은 위치가 아니다 — 읽는 사람이 찾아갈 수 없다
- `본문` / `근거` / `개선 제안` 세 줄을 유지한다. 한 칸으로 합치면 무엇을 왜 어떻게 고쳐야 하는지가 사라진다

위치를 확인하지 못한 finding은 위치 줄 자리에 `위치 미확인 사유: …`를 쓴다 — 줄을 비우거나 서술로 대신하지 않는다.
