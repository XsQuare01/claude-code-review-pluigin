...
- 개별 증상이 흩어져 보여도 **같은 원칙 위반의 공통 원인**일 수 있으면 그 연결을 설명한다. 단, 원칙 이름만으로 등급을 올리지는 않는다
...
|----------|------|
| 높음 | 원칙 위반이 실제로 사용자 오동작, 데이터 손상, 보안 노출, 검증 실패, 외부 계약 파손 중 하나를 낳는다고 현재 변경에서 닫아 말할 수 있는 경우 |
| 낮음 | 구조 혼재, 중복, 표현 과복잡, 유지보수성 저하, 원칙 불일치처럼 설계 비용은 크지만 닫힌 높은 영향 범주를 아직 증명하지 못한 경우 |

원칙 모듈은 **근본 원인과 연결 관계**를 설명하는 데 가치가 있다. 개별 증상보다 더 상위라는 이유만으로 severity를 복사·상향하지 않는다.

규칙 ID는 `10-<원칙 약어>` 형식으로 적는다 (예: `10-SSOT`, `10-SRP`).

---

## 높은 영향도로 이어질 수 있는 원칙 예시
...
## 10-OUTPUT. 도메인 결과 가이드

- 이 모듈은 다른 모듈의 지적을 단순 복사하지 않고, **어떤 증상들이 같은 원칙 위반에서 묶이는지**를 설명해야 한다.
- 원칙 이름만 적지 말고, 지금 변경에서 어떤 trade-off와 근본 원인이 문제인지 적는다.
- 유지보수성·확장성만 나빠진 경우는 기본적으로 영향 낮음 쪽 근거다. 닫힌 높은 영향 범주를 직접 보여주지 못하면 높은 영향도로 올리지 않는다.
