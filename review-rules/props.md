...
Props drilling, 과도한 props 전달, 함수 인자 과다를 독립적으로 검토하는 전용 리뷰 문서. 오직 `/code-review-props`에서만 사용된다. 파일명에 숫자 prefix가 없으므로 일반 `/code-review` 자동 모듈 스캔에서 제외된다.

검사 범위:
- React/Vue/Svelte 등 컴포넌트 props 전달 구조
...
- 🔴 ERROR — 변경으로 인해 props/인자 전달 구조가 명백히 취약해져 수정 비용·버그 위험이 큼
...
부모 → 중간 → 하위로 같은 값이나 핸들러가 3단계 이상 전달되며, 중간 컴포넌트가 그 값을 직접 사용하지 않으면 위반.

지적 기준:
- 중간 컴포넌트가 props를 받자마자 자식에게 그대로 넘김
- 동일 props 이름 또는 동일 의미의 alias가 여러 컴포넌트를 통과
- 상태 변경 핸들러(`onChange`, `onSelect`, `setX`, `handleX`)가 깊게 전달됨
- 특정 leaf 컴포넌트 전용 상태가 상위 페이지/위젯까지 끌어올려짐

개선 방향:
- 실제 사용 위치 가까이 state colocate
- 관련 하위 트리를 합성 컴포넌트로 묶기
- 여러 leaf에서 공유해야 하면 context/store 검토
...
개선 방향:
- wrapper 제거 후 직접 사용
- wrapper가 필요한 이유가 있으면 명확한 책임(레이아웃, 접근성, 정책)을 갖게 분리
- children/render prop/composition으로 전달 경로 단축

## P-3. Props 개수 과다 🟡

하나의 컴포넌트에 props가 7개 이상 전달되면 구조 검토 신호다. 단, 단순 UI primitive처럼 명확한 예외가 있으면 맥락으로 판단한다.
...
상위 상태 setter나 도메인 mutation 핸들러가 여러 단계 아래로 전달되어 leaf가 상위 상태 구조를 알아야 하면 위반.
...
- provider value가 매 렌더 새 객체인데 memoization 없음
...
|----------|------|------|------|------|----------|
| 🔴/🟡/🔵 | path/to/file | line | P-x | 구체적 전달 구조 문제 | 단순화 방향 |
