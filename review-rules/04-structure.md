...
기존 흐름을 정리하지 않은 채 새 조건, 예외, 우회 로직을 덧대어 이후 관리자가 여러 암묵적 관계를 동시에 이해해야 하는 구조는 **변경 비용**과 회귀 위험 기준으로 지적한다. 미관이나 취향 문제가 아니라, 작은 요구사항 변경에도 여러 위치를 함께 수정해야 하거나 경로별 동작이 달라질 수 있는지 확인한다.

다음 신호가 2개 이상 함께 보이면 우선 검토한다:
- UI/데이터 변환/도메인 정책/임시 우회 책임이 한 흐름에 섞여 있음
- 특정 고객/상태/환경만 처리하는 특수 케이스 분기가 계속 누적됨
- 같은 데이터 reshape/normalize가 여러 계층이나 핸들러에서 반복됨
- 이름만 다른 복붙 변형 함수/훅/컴포넌트가 늘어나 정책 반영 위치가 불명확함
- 정상 흐름을 따라가기 어렵고, 어떤 조건에서 어떤 경로가 실행되는지 추적 비용이 큼
- 작은 변경에도 여러 파일/분기/복사본을 동시에 수정해야 함
- error/fallback 처리 기준이 경로마다 달라 같은 실패를 다르게 처리할 수 있음

Severity는 기본적으로 `🟡 WARNING`이다. 단, 중복 경로가 실제로 다르게 실행될 수 있거나 권한/검증/과금/노출 같은 정책 분기가 갈라져 버그 위험이 크면 `🔴 ERROR`로 올릴 수 있다.

개선 방향은 다음 중 실제 비용을 줄이는 쪽으로 제안한다:
- 오래된 경로를 삭제하고 하나의 단일 출처로 통합
- 정책/특수 케이스/fallback 처리를 한 지점으로 중앙화
- UI, 데이터 변환, 도메인 판단, 임시 우회 책임을 분리
- 임시 우회가 필요하면 이유와 제거 조건을 명시

코멘트는 반드시 구체적으로 작성한다: **문제 / 현재 선택 / 왜 위험한지 / 개선 방향**이 드러나야 한다. `코드가 지저분함`, `누더기 같음`, `깔끔하게 정리 필요`, `지저분하다`처럼 변경 비용이나 실제 위험이 없는 표현만으로는 지적하지 않는다.
