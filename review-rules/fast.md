...
>
> 1. **적용 가능성 + 영향도 보정** — 원본의 applicability, exception, typical impact calibration을 유지한다. 원본 모듈의 표기 이모지는 참고 보정값일 뿐이고, fast가 파일별로 고르는 최종 severity는 이미 파생된 finding severity를 사용한다.
...
- 🔴/🟡/🔵 는 **이미 두 축에서 파생된 finding severity**를 뜻한다. fast는 규칙 표기의 색을 그대로 복사하지 않고, 선택된 finding의 severity를 사용한다.
- 규칙 옆 고정 이모지는 여전히 **비구속적 보정 마커**다. applicability와 typical impact를 압축본에 유지하기 위한 참고값이지, 최종 등급 결정권이 아니다.
...
- 🔴 위치는 파일을 읽어 확인한 **변경 후 줄 번호** + 그 줄 코드 한 줄 인용. `@@` 헤더에서 계산하지 않는다. 확인 불가면 `위치 미확인` (00-10)
- 🔴 "없다·가능하다" 형태의 지적에는 **무엇을 확인했는지** 함께 적는다. 확인 범위 밖이면 지적 대신 `확인 필요` (00-11)
- 리포트 섹션 이름·순서·헤딩 레벨은 `workflow-contract.md` C-7 문서 골격을 따른다. 같은 ID로 지적이 둘 이상이면 `03-1 (1/2)` 순번 (00-2)
